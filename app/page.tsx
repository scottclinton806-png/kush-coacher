...
const VEG_SCHEDULE: WeekSchedule[] = [
  ...
  },
  {
    ...
    nutrients: [
      { name: "Bulletproof SI", unit: "mL/gal", dose: 2.5 },
      { name: "Base Nutrients", unit: "g/gal", dose: 3 },
      { name: "Epsom Salt", unit: "g/gal", dose: 1 },
      { name: "Cal-Nit (15-0-0)", unit: "g/gal", dose: 2 },
      { name: "Great White", unit: "tsp/10gal", dose: 1 },
...
const FLOWER_SCHEDULE: WeekSchedule[] = [
  {
    ...
    nutrients: [
      ...
      { name: "Epsom Salt", unit: "g/gal", dose: 2.5 },
      { name: "Cal-Nit (15-0-0)", unit: "g/gal", dose: 2.5 },
      { name: "Cutting Edge Bloom (0-6-5)", unit: "mL/gal", dose: 10 },
      { name: "Great White", unit: "tsp/10gal", dose: 1 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 10 },
    ]
  },
  {
  ...
  },
  {
    ...
    nutrients: [
      { name: "Bulletproof SI", unit: "mL/gal", dose: 2.5 },
      { name: "Base Nutrients", unit: "g/gal", dose: 3.6 },
      { name: "Epsom Salt", unit: "g/gal", dose: 3.5 },
      { name: "Cal-Nit (15-0-0)", unit: "g/gal", dose: 2.4 },
      { name: "Great White", unit: "tsp/10gal", dose: 1 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 10 },
    ]
  },
  {
    ...
    nutrients: [
      ...
      { name: "Epsom Salt", unit: "g/gal", dose: 3.5 },
      { name: "Cal-Nit (15-0-0)", unit: "g/gal", dose: 2.4 },
      { name: "Cutting Edge Bloom (0-6-5)", unit: "mL/gal", dose: 15 },
      { name: "Great White", unit: "tsp/10gal", dose: 1 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 10 },
    ]
  },
  {
    ...
    nutrients: [
      ...
      { name: "Base Nutrients", unit: "g/gal", dose: 3.6 },
      { name: "Epsom Salt", unit: "g/gal", dose: 3.5 },
      { name: "Cal-Nit (15-0-0)", unit: "g/gal", dose: 1.5 },
      { name: "Cutting Edge Bloom (0-6-5)", unit: "mL/gal", dose: 15 },
      { name: "Great White", unit: "tsp/10gal", dose: 1 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 10 },
    ]
  },
  {
    ...
    nutrients: [
      ...
      { name: "Cutting Edge Bloom (0-6-5)", unit: "mL/gal", dose: 15 },
      { name: "Great White", unit: "tsp/10gal", dose: 1 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 20 },
    ]
  },
  {
    ...
    nutrients: [
      { name: "Base Nutrients", unit: "g/gal", dose: 2 },
      { name: "Epsom Salt", unit: "g/gal", dose: 1 },
      { name: "Cutting Edge Bloom (0-6-5)", unit: "mL/gal", dose: 5 },
      { name: "Shooting Powder", unit: "g/gal", dose: 2.6 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 20 },
    ]
  },
  {
    ...
    nutrients: [
      ...
      { name: "Epsom Salt", unit: "g/gal", dose: 1 },
      { name: "Cutting Edge Bloom (0-6-5)", unit: "mL/gal", dose: 5 },
      { name: "Shooting Powder", unit: "g/gal", dose: 5.2 },
      { name: "Full Power Humic", unit: "mL/gal", dose: 20 },
    ]
  },
  {
    week: "Flower Week 9", targetEC: "0", pH: "6.1–6.3",
    nutrients: [
      { name: "Shooting Powder", unit: "g/gal", dose: 5.2 },
    ]
  },
...
const MIXING_ORDER = [
  { step: "1", label: "Bulletproof SI", note: "Add FIRST — before adding anything else" },
  ...
  { step: "4", label: "Cal-Nit (15-0-0)", note: "Add next — mix until dissolved" },
  { step: "5", label: "Cutting Edge Bloom / Shooting Powder", note: "Add next — mix well" },
  { step: "6", label: "Full Power Humic", note: "Flower only — add very last" },
  { step: "7", label: "Check pH & EC/PPM", note: "Adjust pH as needed. Add RO water if EC is too high." },
];
...
export default function KushCoacher() {
  ...
  const schedule: WeekSchedule[] = phase === "veg" ? VEG_SCHEDULE : FLOWER_SCHEDULE;
  const currentWeek: WeekSchedule = schedule[weekIndex];
  const gal: number = parseFloat(gallons) || 0;
  const phaseColor: string = phase === "veg" ? "#4ade80" : "#fb923c";
  const phaseDark: string = phase === "veg" ? "#052e16" : "#1c0700";

  function calcDose(dose: number): string | null {
    if (!gal || !dose) return null;
    return (dose * gal).toFixed(2);
  }

  function openNotes(weekLabel: string): void {
    const key: string = weekLabel;
  ...
  function saveNote(): void {
    ...
    setGrowNotes((prev: Record<string, string>) => ({ ...prev, [activeNotesKey as string]: noteInput }));
    setNoteSaved(true);
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px 60px" }}>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {schedule.map((w: WeekSchedule, i: number) => {
              const hasNote: boolean = !!growNotes[w.week];
              return (
        ...
        <div style={{
          ...
          {currentWeek.nutrients.filter((n: Nutrient) => !n.skip).map((n: Nutrient, i: number) => {
            const visibleNutrients: Nutrient[] = currentWeek.nutrients.filter((n: Nutrient) => !n.skip);
            const isLast: boolean = i === visibleNutrients.length - 1;
            const isGreatWhite: boolean = n.name === "Great White";
            let displayTotal: string | null = null;

            if (isGreatWhite) {
              // Great White: 1 heaping tsp per 10 gallons
              if (gal > 0) {
                const tsps: number = gal / 10;
                displayTotal = tsps.toFixed(2);
              }
            } else {
              displayTotal = calcDose(n.dose);
            }
              ...
              <div key={i} style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#444", marginTop: 2 }}>
                    {isGreatWhite ? "1 heaping tsp / 10 gal" : `${n.dose} ${n.unit}`}
                  </div>
                </div>
                  ...
                  {displayTotal ? (
                    <>
                      ...
                      <div style={{ fontSize: 11, color: "#555" }}>
                        {isGreatWhite ? "tsp" : n.unit.replace("/gal", "")}
                      </div>
                    </>
                  ) : (
